<?php
//puntenPerPersoonOverzicht.php

declare(strict_types=1);
require_once 'PuntLijst.php';
require_once 'PersoonLijst.php';

$puntlijst = new PuntLijst();

if (isset($_GET["action"]) && $_GET["action"] === "viewpersoon") {
    $perpersoon = $puntlijst->getPuntenPerPersoonLijst((int) $_POST["selPersonen"]);
}

//===================================================================

if (isset($_GET["action"]) && $_GET["action"] === "delete") {
  $puntlijst->deletePunt((int)$_GET["id"]);
}

//===================================================================


?>

<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Punten</title>
  </head>
<?php include_once("nav.php"); ?>
    <section>
      <div>
            <h1>Punten persoon</h1>
            <table>
                <thead>
                    <tr>
                        <th scope="col">Naam</th>
                        <th scope="col">Module</th>
                        <th scope="col">Score</th>
                        <th scope="col"></th>
                        <th scope="col"></th>                         
                    </tr>
                </thead>
                <tbody>          
            <?php
            if (isset($perpersoon)){
              foreach ($perpersoon as $punt) {
                ?>
                    <tr>
                        <td><?php print $punt->getPersoon()->getNaam();?></td>
                        <td><?php print $punt->getModule()->getNaam();?></td>
                        <td><?php print $punt->getPunt();?></td>
                        <td><a href="puntBewerken.php?id=<?php print $punt->getId();?>">bewerken</a></td>
                        <td><a href="puntenPerPersoonOverzicht.php?action=delete&id=<?php print($punt->getId()); ?>">delete</a></td>                        
                    </tr>                    
                    
                <?php
              }
            }
            ?>
        
                </tbody>
          </table>                    
        </div>
        </section>
<section>                    
      <div>      
        <h1>Zoek persoon</h1>       
        <?php 
           $persoonlijst = new PersoonLijst();
           $tabpersoon = $persoonlijst->getLijst();        
        ?>         
        <form action="puntenPerPersoonOverzicht.php?action=viewpersoon" method="post">
           <select name="selPersonen">
               <?php foreach ($tabpersoon as $persoon) { ?>
                  <option value="<?php print $persoon->getId(); ?>"><?php print $persoon->getNaam(); ?></option>
               <?php } ?>
           </select>
            <input type="submit" value="Zoek" />
        </form>
        </div>
        </section>    
    </body>
</html>
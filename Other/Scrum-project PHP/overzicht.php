...
if (!empty($zoekterm) && (strlen($zoekterm) < 3)) {
    ...
    $artikels = $artikelService->getArtikelenSpecifiek($paginaNummer, $maxAantalArtikelsPerPagina, $zoekterm, $categorieId, $hoofdCategorieId, $subCategorieId, $sorteerOp, $sorteerRichting);
    if (empty($artikels)) {
        $geenResultaten = true;
    }
}
